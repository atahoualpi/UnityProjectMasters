...
[CustomEditor(typeof(AudioManager_FilterBased))]
[CanEditMultipleObjects]
public class AudioManager_FilterBased_Editor : Editor
{
    [MenuItem("PhysicsBasedAudio/FilterBasedAudio")]
    static void CreateFilterBasedAudioManager()
    {
        GameObject target = Selection.activeGameObject;
        if (target != null)
        {
            target.AddComponent<AudioManager_FilterBased>();
        }
    }

    AudioManager_FilterBased scriptTarget;

    private void OnEnable()
    {
        scriptTarget = (AudioManager_FilterBased)target;
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.EndHorizontal();
        GUILayout.BeginHorizontal();
        float material = scriptTarget.HeavyScript.GetFloatParameter(Hv_FilterRolling_AudioLib.Parameter.Qfactor);
        float newMaterial = EditorGUILayout.Slider("", material, 20f, 5000f);
        if (material != newMaterial)
        {
            scriptTarget.HeavyScript.SetFloatParameter(Hv_FilterRolling_AudioLib.Parameter.Qfactor, newMaterial);
        }
        GUILayout.EndHorizontal();
        GUILayout.BeginHorizontal();
        ...
        if (size != newSize)
        {
            scriptTarget.HeavyScript.SetFloatParameter(Hv_FilterRolling_AudioLib.Parameter.Size, 2 - newSize);
        ...
        float roughness = scriptTarget.HeavyScript.GetFloatParameter(Hv_FilterRolling_AudioLib.Parameter.Object_roughness);
        float newRoughness = EditorGUILayout.Slider("", roughness, 0f, 50f);
        if (roughness != newRoughness)
        {
            scriptTarget.HeavyScript.SetFloatParameter(Hv_FilterRolling_AudioLib.Parameter.Object_roughness, newRoughness);
        }
